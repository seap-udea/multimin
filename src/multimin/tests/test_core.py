...
def test_cmnd_reset_params_updates_mus_only():
...
def test_cmnd_reset_params_updates_sigmas_and_rhos():
    ...
    cmnd.reset_params(sigmas=[2.0, 3.0], rhos=[0.25])
...
def test_cmnd_reset_params_broadcasting_and_shape_errors():
    """reset_params enforces FitCMND-like shapes."""
    cmnd = mn.ComposedMultiVariateNormal(
        weights=[0.5, 0.5],
        mus=[[0.0, 0.0], [1.0, 1.0]],
        Sigmas=[np.eye(2), np.eye(2)],
    ...
    with pytest.raises(ValueError):
        cmnd.reset_params(mus=[1.0])

    with pytest.raises(ValueError):
        cmnd.reset_params(sigmas=[[1.0, 2.0, 3.0], [1.0, 2.0, 3.0]])

    with pytest.raises(ValueError):
        cmnd.reset_params(rhos=[0.1, 0.2])


def test_cmnd_reset_params_updates_weights_and_normalizes():
    ...
    cmnd.reset_params(weights=[2.0, 1.0])
    assert np.allclose(cmnd.weights, np.array([2.0 / 3.0, 1.0 / 3.0]))
    assert np.isclose(cmnd.weights.sum(), 1.0)


def test_cmnd_reset_params_updates_weights_without_normalization():
