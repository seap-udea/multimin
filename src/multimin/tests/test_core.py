...
def test_plotting(setup_gallery, mog_params):
    ...
    scatter = G.sample_scatter(data, **sargs)
    assert scatter is not None, "Scatter plot returned None"
...
def test_mog_reset_params_updates_mus_only():
    """reset_params updates mus without altering Sigmas."""
    mog = mn.MixtureOfGaussians(
        weights=[0.5, 0.5],
        mus=[[0.0, 0.0], [1.0, 1.0]],
...
def test_mog_reset_params_updates_sigmas_and_rhos():
    """reset_params updates sigmas/rhos and recomputes covariance matrices."""
    mog = mn.MixtureOfGaussians(
        weights=[0.2, 0.8],
        mus=[[0.0, 0.0], [1.0, 1.0]],
        Sigmas=[np.eye(2), np.eye(2)],
...
def test_mog_reset_params_broadcasting_and_shape_errors():
    """reset_params enforces FitMoG-like shapes."""
    mog = mn.MixtureOfGaussians(
        weights=[0.5, 0.5],
        mus=[[0.0, 0.0], [1.0, 1.0]],
        Sigmas=[np.eye(2), np.eye(2)],
...
def test_mog_reset_params_updates_weights_and_normalizes():
    """reset_params updates weights and normalizes when normalize_weights=True."""
    mog = mn.MixtureOfGaussians(
        weights=[0.5, 0.5],
        mus=[[0.0, 0.0], [1.0, 1.0]],
        Sigmas=[np.eye(2), np.eye(2)],
...
def test_mog_reset_params_updates_weights_without_normalization():
    """reset_params keeps weights scale when normalize_weights=False."""
    mog = mn.MixtureOfGaussians(
        weights=[0.5, 0.5],
        mus=[[0.0, 0.0], [1.0, 1.0]],
        Sigmas=[np.eye(2), np.eye(2)],
...
def test_function_fit_r2_statistic():
    """Test R² calculation for univariate function fitting (FitFunctionMoG)."""
    # Create a simple univariate Gaussian function
    np.random.seed(42)
    xs = np.linspace(-3, 3, 100)
    # True function: scaled Gaussian
    ys_true = 5.0 * np.exp(-0.5 * ((xs - 0.5) / 0.8) ** 2)
    # Add small noise
    ys = ys_true + np.random.normal(0, 0.1, size=ys_true.shape)

    # Fit with a single Gaussian
    F = mn.FitFunctionMoG(data=(xs, ys), ngauss=1)
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message="divide by zero encountered in log")
        F.fit_data(verbose=0, options={"maxiter": 100})

    # Calculate quality of fit
    stats = F.quality_of_fit(verbose=False)

    # Assertions
    assert "R2" in stats, "quality_of_fit should return R2 key"
    assert np.isfinite(stats["R2"]), "R² should be finite"
    assert stats["R2"] > 0.8, (
        f"R² should be high for good fit (got {stats['R2']:.3f})"
    )
    assert stats["R2"] <= 1.0, "R² should not exceed 1.0"


def test_multivariate_fit_ks_statistic():
    """Test K-S calculation for multivariate fitting (FitMoG)."""
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message="divide by zero encountered in log")
        F.fit_data(data, verbose=0, options={"maxiter": 100})

    # Calculate quality of fit (K-S and R² identity)
    stats = F.quality_of_fit(data=data, n_sim=5000, verbose=False)

    # Assertions
    assert "ks_dist" in stats, "quality_of_fit should return ks_dist key"
    assert "r2_identity" in stats, "quality_of_fit should return r2_identity key"
    assert np.isfinite(stats["ks_dist"]), "K-S distance should be finite"
    assert np.isfinite(stats["r2_identity"]), "R² identity should be finite"
    assert 0.0 <= stats["ks_dist"] <= 1.0, "K-S distance should be in [0, 1]"
    assert stats["ks_dist"] < 0.1, (
        f"K-S distance should be low for good fit (got {stats['ks_dist']:.3f})"
    )
    assert stats["r2_identity"] > 0.8, (
        f"R² identity should be high for good fit (got {stats['r2_identity']:.3f})"
    )
